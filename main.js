...
const login = async (instance, usr, password) => {
  ...
  if (!(await instance.$("#headerlink-logout"))) {
...
const updateBuilding = async (instance, building) => {
  ...
  try {
    const buildingRow = await instance.$(
      `#main_buildrow_${building} > td:nth-child(7) > a:nth-child(2)`
    );
    await buildingRow.click();
    Promise.resolve()
    console.log(`+ updating ${building} +`.green);
  } catch {
    console.log(`- Can't update ${building} -`.red);
  }
};
